...
export default function AccessModal({ isOpen, onClose, onVerified, type }) {
    ...
    const handleSubmit = (e) => {
        ...
        setTimeout(() => {
            if (code === codes.admin && type === 'admin') {
                onVerified('admin')
                setCode('')
                onClose()
            } else if (code === codes.staff || code === codes.faculty) {
                const verifiedRole = code === codes.staff ? 'staff' : 'faculty'
                ...
                <div className="modal-header">
                    <div className="flex items-center gap-2">
                        ...
                        <h2 className="modal-title" style={{ fontSize: 'var(--font-base)', letterSpacing: '0.05em' }}>
                            {type === 'admin' ? 'SECURE_ADMIN_GATEWAY' : 'STAFF_FACULTY_AUTH_PORTAL'}
                        </h2>
                    </div>
