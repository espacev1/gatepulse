...
import { 
    ...
    Users2, Info, X, Filter, Globe, Download, Lock
} from 'lucide-react'
...
export default function FacultyEvents() {
    ...
    useEffect(() => {
        ...
        const subscription = supabase
            .channel('faculty-events-synopsis-live')
    ...
    useEffect(() => {
        if (userProfile?.dept && userProfile.dept !== 'GLOBAL' && userProfile.dept !== 'WAITING_FOR_REGISTRATION') {
    ...
    const exportParticipantsCSV = async (event) => {
        ...
        if (!data || data.length === 0) return alert('No participants found for this event.')
    ...
    const exportJuryMarksCSV = async (event) => {
        ...
        if (!data || data.length === 0) return alert('No jury marks found for this event.')

        const csvRows = [
            ['Participant (Team/Solo)', 'Technical', 'Innovation', 'Presentation', 'Usability', 'Total Marks'],
            ...data.map(m => [
    ...
    const exportCheckinsCSV = async () => {
        ...
        const headers = ['Name', 'Registration ID', 'Department', 'Timestamp']
    ...
    return (
        <div className="page-container">
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 'var(--space-6)', background: 'var(--bg-mid)', padding: 'var(--space-4)', borderRadius: 'var(--radius-xl)', border: '1px solid var(--border-color)' }}>
                <div>
                    <h1 className="page-title" style={{ margin: 0 }}>Faculty: Event Overview</h1>
                    <p className="page-subtitle">Monitor event registrations and attendance in real-time.</p>
                </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)', overflow: 'hidden' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)', background: 'var(--bg-card)', padding: 'var(--space-4)', borderRadius: 'var(--radius-lg)', border: '1px solid var(--border-color)' }}>
                        <div className="flex items-center justify-between">
                            ...
                            {userProfile?.dept && userProfile.dept !== 'GLOBAL' && (
                                <div className="flex items-center gap-1 text-[8px] text-accent font-bold uppercase tracking-widest bg-accent-glow px-2 py-0.5 rounded">
                                    <Lock size={8} /> RESTRICTED_BY_ADMIN
                                </div>
                            )}
                        </div>
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                            <button
                                ...
                                disabled={userProfile?.dept && userProfile.dept !== 'GLOBAL'}
                            >
                            ...
                            {DEPARTMENTS.map(dept => (
                                <button
                                    ...
                                    style={{ fontSize: '9px', padding: '2px 6px' }}
                                    disabled={userProfile?.dept && userProfile.dept !== 'GLOBAL' && userProfile.dept !== dept}
                                >
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)', background: 'var(--bg-card)', padding: 'var(--space-4)', borderRadius: 'var(--radius-lg)', border: '1px solid var(--border-color)' }}>
                        <div className="flex items-center gap-2">
                            ...
                            <span style={{ fontSize: '10px', fontWeight: 800, color: 'var(--text-dim)', letterSpacing: '0.05em' }}>CATEGORY_FILTER</span>
                        </div>
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                        ...
                        {filtered.map(event => (
                            ...
                            >
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '10px', color: 'var(--text-dim)', fontFamily: 'var(--font-mono)' }}>
                                    ...
                                    <span>{Math.round(((event.registered_count || 0) / (event.max_capacity || 1)) * 100)}% FULL</span>
                                </div>
